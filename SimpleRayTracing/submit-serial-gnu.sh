...
/usr/bin/time --format='%e' ./bin-gnu/main --size $width $height --jpeg serial-gnu-${width}x$height.jpg 2> temp-serial-gnu-${width}x$height

RUNTIME=`cat temp-serial-gnu-${width}x$height`
rm -f temp-serial-gnu-${width}x$height
